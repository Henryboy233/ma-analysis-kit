...
def business_due_diligence():
    ...
    if 'dd_integration' not in st.session_state:
        st.session_state.dd_integration = None
    ...
    if st.button("🚀 开始商业尽调", type="primary"):
        ...
        with st.spinner("正在深度分析..."):
            # 执行分析并保存结果
            report_data, file_contents, dd_integration = perform_dd_analysis(target_name, industry, uploaded_files, analysis_dims)
            st.session_state.dd_report_data = report_data
            st.session_state.dd_file_contents = file_contents
            st.session_state.dd_integration = dd_integration
    ...
    if st.session_state.dd_analysis_done and st.session_state.dd_target_name == target_name:
        display_dd_results(
            ...
            st.session_state.dd_integration
        )
...
"""
    ...
    if st.button("🚀 AI增强尽调分析", type="primary", help="调用Gemini AI进行流式分析（实时显示）"):
        # 创建结果展示区域
        st.markdown("---")
        st.markdown("### 📊 AI深度分析中...")
        ...
        stream_placeholder.info("⏳ 正在连接AI服务...")
        ...
        try:
            ...
            stream_placeholder.markdown("🤖 *AI正在思考...*")
            
            # 流式获取AI分析结果
            full_response = ""
            chunk_count = 0
            
            for chunk in gemini.analyze_due_diligence_stream(
                ...
                chunk_count += 1
                # 更新进度条（模拟进度）
                progress = min(0.9, chunk_count / 50)  # 假设约50个chunk
                progress_bar.progress(progress)
            ...
            # 检查是否返回了有效内容
            if full_response and not full_response.startswith("API调用失败"):
                # 保存到session_state
                st.session_state.ai_analysis_result = full_response
                st.session_state.ai_analysis_company = company_name
                
                # 显示完成提示
                st.success(f"✅ AI分析完成！共生成 {len(full_response)} 字符")
...
"""
                ...
                st.error("❌ AI分析未返回有效结果，请重试")
                
        except Exception as e:
            st.error(f"❌ AI分析失败: {str(e)}")
            st.info("💡 可能原因：API服务繁忙或网络问题，请稍后重试")
    
    # 显示历史AI分析结果（如果有缓存）
    elif st.session_state.ai_analysis_result and st.session_state.ai_analysis_company == company_name:
        st.markdown("---")
        st.markdown("### 📊 AI深度分析报告（缓存）")
